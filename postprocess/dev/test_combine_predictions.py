...
the aim of this code is going to be to:
    ...
    if they do not exist, generate a spectrogram using the label file (when predicting on files with no labels, this will not be needed)
    loop through all the spectrograms, predict and derive a label from the prediction
    put all the labels together into a label table which will be comparable
"""
