...
for i in range(0,len(models_paths)):    
    ...
    for low_thr in [0.2]:#[0.1,0.3]:
        for high_thr in [0.5,0.7,0.9]: 
            ...
            predictions_list = [os.path.join(root_paths[i], "pred_table", file_ID + "_PRED_TABLE_thr_" + str(low_thr) + "-" + str(high_thr) + ".txt" ) for file_ID in file_ID_list ]
            evaluation = metrics.Evaluate(label_list, predictions_list, 0.5, 5) # 0.99 is 0.5
    ...
    for low_thr in [0.2]:
        for high_thr in [0.5,0.7,0.9]: 
            ...
            for file_ID in testing_filenames:
                ...
                pred_table.to_csv(os.path.join(root_paths[i], "pred_table", save_pred_table_filename), header=True, index=None, sep=';')
