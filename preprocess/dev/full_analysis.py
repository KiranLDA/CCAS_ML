...
save_data_path = '/media/kiran/D0-P1/animal_data/meerkat/new_run_sep_2020/'
...
if os.path.exists(os.path.join(save_label_table_path, "training_files_used.txt")):
    # load the saved file
...
This section has been commented out, because at the moment, it is not necessary to run the test
set. Instead it is done live in the postprocessing
'''
...
spec_filepaths = [os.path.join(save_spec_train_path, x) for x in os.listdir(save_spec_train_path)] #[x for x in os.listdir(spectro_folder)]
for calltype in call_types.keys

# noise aug
augmented_data, augmented_spectrogram, augmented_label, spec_name, label_name = pre.augment_with_noise(spec_filepaths, audio_filepaths, calltype, noise, scaling_factor, 
                                                                                random_range, spec_window_size, n_mels, window, fft_win, fft_hop, normalise,
                                                                                save_label_table_path, call_types,  label_for_noise)

# pitch shift
augmented_data, augmented_spectrogram, augmented_label, spec_name, label_name = pre.augment_with_pitch_shift(spec_filepaths, audio_filepaths, calltype, n_steps,
                                                                                random_range, spec_window_size, n_mels, window, fft_win, fft_hop, normalise,
                                                                                save_label_table_path, call_types, label_for_noise)
...
augmented_data, augmented_spectrogram, augmented_label, spec_name, label_name = pre.augment_with_time_stretch(spec_filepaths, audio_filepaths, calltype, stretch_factor,
                                                                                random_range, spec_window_size, n_mels, window, fft_win, fft_hop, normalise,
                                                                                save_label_table_path, call_types, label_for_noise)
...
#----------------------------------------------------------------------------------------------------
#          Weigh the calls according to the numbers available
#----------------------------------------------------------------------------------------------------
...
for i in weight_dict:
    weight_dict[i] = float(weight_dict[i]/max_call)
...
sf = os.path.join(save_model_path,"model_shuffle_test_" + date_now + "_" + time_now)
